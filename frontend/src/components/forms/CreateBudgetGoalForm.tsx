...
export const CreateBudgetGoalForm: React.FC<CreateBudgetGoalFormProps> = ({
  isOpen,
  onClose,
  onSuccess
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ 
        submit: error.response?.data?.error || 'Failed to create goal. Please try again.' 
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-300">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto transform transition-all duration-300 animate-in slide-in-from-bottom-4">
        <Card className="relative">
          ...
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Create Savings Goal</h2>
            <p className="text-gray-600 dark:text-gray-300">Set up a new savings goal to track your progress</p>
          </div>
